    ...
    module Update
      def save(opts={})
        ...
        if values.length > 0
          ...
          response, api_key = Freshdesk.request(:post, url, @api_key, values)
      ...
      def serialize_metadata
        if @unsaved_values.include?(:metadata)
          ...
          # metadata is a FreshdeskObject, and can be serialized normally
          serialize_params(@values[:metadata])
        end
      end

      def serialize_params(obj)
        if obj.class.name.include?('Freshdesk')
          obj = obj['helpdesk_ticket'] ? obj[:helpdesk_ticket] : obj
        end
        ...
        when FreshObject
          ...
          update_hash = { :helpdesk_ticket => {} }

          unsaved_keys.each do |k|
            update_hash[:helpdesk_ticket][k] = serialize_params(obj_values[k])
          end
